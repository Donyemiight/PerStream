  ...
  function fixCreatorUpload() {
    ...
    const obs = new MutationObserver(() => {
      ...
      cards.forEach(card => {
        ...
        console.log('[perstream-fixes] card', id, 'captured:', card.__perstream_captured);
        if (!id) return;
        ...
        const allUserTracks = getUserTracks();
        const existing = allUserTracks.find(t => t.id === id);
        console.log('[perstream-fixes] card', id, 'existing?', !!existing, 'allUserTracks:', allUserTracks.map(t => t.id), 'raw:', localStorage.getItem('perstream_user_tracks'));
        if (existing) {
